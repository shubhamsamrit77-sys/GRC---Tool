...
function renderEmailTracker(){
  ...
  if(sumEl){
    sumEl.innerHTML='';
    var metrics=[
      {label:'Total emails',val:total,color:'var(--text)',icon:'📧',bg:'var(--surface2)'},
      {label:'Waiting reply',val:waiting,color:'var(--primary)',icon:'⏳',bg:'var(--primary-light)'},
      {label:'No reply — overdue',val:overdue,color:'var(--danger)',icon:'🔴',bg:'var(--danger-light)'},
      {label:'Replied',val:replied,color:'var(--success)',icon:'✅',bg:'var(--success-light)'}
    ];
    metrics.forEach(function(m){
      ...
      card.innerHTML='<div class="mc-label">'+m.label+'</div>'+
  ...
  if(!filtered.length){
    ...
    var td0=document.createElement('td');
    td0.colSpan=7;
    td0.style.cssText='padding:48px 20px;text-align:center';
    if(total===0){
      td0.innerHTML='<div style="font-size:32px;margin-bottom:10px">📬</div>'+
        '<div style="font-size:14px;font-weight:600;color:var(--text);margin-bottom:6px">No emails tracked yet</div>'+
        '<div style="font-size:13px;color:var(--text3);margin-bottom:16px">Click Compose &amp; Track to start sending and tracking compliance emails</div>'+
        '<button class="btn btn-primary" onclick="openCompose()" style="margin:0 auto">+ Compose &amp; Track</button>';
    } else {
      td0.innerHTML='<div style="font-size:28px;margin-bottom:8px">🔍</div>'+
        '<div style="font-size:13px;color:var(--text3)">No emails match this filter</div>';
    }
  ...
  filtered.forEach(function(e){
    ...
    var daysOverdue=diff!==null?Math.abs(diff):0;

    var tr=document.createElement('tr');
    tr.className='item-row';
    tr.style.height='60px';
    if(isOverdue) tr.style.background='rgba(239,68,68,.03)';

    // Col 1: Subject + category
    var td1=document.createElement('td');
    td1.style.paddingLeft='16px';
    var subDiv=document.createElement('div');
    subDiv.style.cssText='font-weight:600;color:var(--text);overflow:hidden;text-overflow:ellipsis;white-space:nowrap;max-width:200px';
    subDiv.textContent=e.subject;
    subDiv.title=e.subject;
    var catDiv=document.createElement('div');
    catDiv.style.cssText='font-size:11px;color:var(--text3);margin-top:2px';
    catDiv.textContent=e.category||'';
    td1.appendChild(subDiv);td1.appendChild(catDiv);

    // Col 2: Stakeholder
    var td2=document.createElement('td');
    td2.innerHTML='<span style="font-weight:600;color:var(--text)">'+e.stakeholder+'</span>';

    // Col 3: Email
    var td3=document.createElement('td');
    var emailLink=document.createElement('a');
    emailLink.href='mailto:'+e.email;
    emailLink.style.cssText='font-size:12px;color:var(--primary);text-decoration:none;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;display:block;max-width:130px';
    emailLink.textContent=e.email||'—';
    emailLink.title=e.email||'';
    td3.appendChild(emailLink);

    // Col 4: Sent
    var td4=document.createElement('td');
    td4.style.cssText='font-size:12px;color:var(--text2)';
    td4.textContent=e.date_sent||'—';

    // Col 5: Deadline
    var td5=document.createElement('td');
    if(e.response_deadline){
      var dColor=isOverdue?'var(--danger)':isDueSoon?'var(--warning)':'var(--text2)';
      td5.innerHTML='<div style="font-size:12px;font-weight:'+(isOverdue||isDueSoon?'700':'400')+';color:'+dColor+'">'+e.response_deadline+'</div>';
      if(isOverdue) td5.innerHTML+='<div style="font-size:10px;color:var(--danger);font-weight:700;margin-top:2px">'+daysOverdue+'d overdue</div>';
      else if(isDueSoon) td5.innerHTML+='<div style="font-size:10px;color:var(--warning);font-weight:700;margin-top:2px">Due tomorrow</div>';
    } else {
      td5.innerHTML='<span style="color:var(--text3)">—</span>';
    }
    ...
    if(e.replied){
      badge.style.cssText='background:var(--success-light);color:var(--success)';
      badge.textContent='✓ Replied';
      var rdDiv=document.createElement('div');
      rdDiv.style.cssText='font-size:10px;color:var(--success);margin-top:3px;font-weight:500';
      rdDiv.textContent=e.reply_date?'on '+e.reply_date:'';
      td6.appendChild(badge);td6.appendChild(rdDiv);
    } else if(isOverdue){
      badge.style.cssText='background:var(--danger-light);color:var(--danger)';
      badge.textContent='✗ No reply';
      td6.appendChild(badge);
    } else if(isDueSoon){
      badge.style.cssText='background:var(--warning-light);color:var(--warning)';
      badge.textContent='⚡ Due soon';
      td6.appendChild(badge);
    } else {
      badge.style.cssText='background:var(--primary-light);color:var(--primary)';
    ...
    actWrap.style.cssText='display:flex;gap:5px;flex-wrap:wrap;align-items:center';

    if(!e.replied){
      // MAIN ACTION: Mark as Replied - big green button
      var rBtn=document.createElement('button');
      rBtn.className='btn btn-sm';
      rBtn.style.cssText='background:var(--success-light);border-color:rgba(16,185,129,.3);color:var(--success);font-weight:700;font-size:11px';
      rBtn.innerHTML='<svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"><polyline points="20 6 9 17 4 12"/></svg> Mark Replied';
      rBtn.title='Click when you receive their reply';
      (function(eid, subj){
        rBtn.onclick=function(e2){ e2.stopPropagation(); showReplyConfirm(eid, subj); };
      })(e.id, e.subject);
      actWrap.appendChild(rBtn);

      // Open Gmail button
      var gBtn=document.createElement('button');
      gBtn.className='btn btn-sm';
      gBtn.style.fontSize='11px';
      gBtn.title='Open Gmail to send or follow up';
      gBtn.innerHTML='<svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z"/><polyline points="22,6 12,13 2,6"/></svg> Gmail';
      (function(em,sub){
        gBtn.onclick=function(e2){
          e2.stopPropagation();
          window.open('https://mail.google.com/mail/?view=cm&fs=1&to='+encodeURIComponent(em)+'&su='+encodeURIComponent('Re: '+sub),'_blank');
        };
      })(e.email||'',e.subject||'');
      actWrap.appendChild(gBtn);
    } else {
      // Undo button for replied
      var uBtn=document.createElement('button');
      uBtn.className='btn btn-sm';
      uBtn.style.fontSize='11px';
      uBtn.textContent='Undo reply';
      (function(eid){ uBtn.onclick=function(e2){e2.stopPropagation();markEmailReplied(eid,true);}; })(e.id);
      actWrap.appendChild(uBtn);
    }

    // Delete button
    var dBtn=document.createElement('button');
    dBtn.className='btn btn-sm';
    dBtn.style.cssText='font-size:11px;border-color:rgba(239,68,68,.25);color:var(--danger)';
    dBtn.textContent='Delete';
    (function(eid){ dBtn.onclick=function(e2){e2.stopPropagation();deleteTrackedEmail(eid);}; })(e.id);
    actWrap.appendChild(dBtn);
...
// Show reply confirmation modal
function showReplyConfirm(id, subject){
  var modal=document.getElementById('reply-confirm-modal');
  var subEl=document.getElementById('reply-confirm-subject');
  var dateEl=document.getElementById('reply-date-input');
  var confirmBtn=document.getElementById('confirm-reply-btn');
  if(!modal) return;
  if(subEl) subEl.textContent='"'+subject+'"';
  if(dateEl) dateEl.value=today();
  if(confirmBtn){
    confirmBtn.onclick=function(){
      var replyDate=dateEl?dateEl.value:today();
      markEmailRepliedWithDate(id, replyDate);
      closeReplyConfirm();
    };
  }
  modal.style.display='flex';
...
async function markEmailRepliedWithDate(id, replyDate){
  var updateBody={replied:true, reply_date:replyDate||today()};
  var res=await api('grc_email_tracker?id=eq.'+id,{method:'PATCH',body:updateBody,extra:{'Prefer':'return=minimal'}});
  if(res&&res.ok){
    await loadTrackedEmails();
    renderEmailTracker();
    showDueDateToast('','✅ Reply recorded! Email marked as complete.');
  }
}

function markEmailReplied(id,undo){
  var newVal=undo?false:true;
  var updateBody={replied:newVal,reply_date:newVal?today():null};
  api('grc_email_tracker?id=eq.'+id,{method:'PATCH',body:updateBody,extra:{'Prefer':'return=minimal'}}).then(function(res){
    if(res&&res.ok){loadTrackedEmails().then(function(){renderEmailTracker();});}
  });
}
...
function onSearchInput(val){
  var clearBtn = document.document.createElement('div');
...
// ── Collect every event from all data sources ──
  ...
  if(!filterType || filterType === 'all' || filterType === 'action'){
    actions.forEach(function(ac){
      ...
      evts.push({
        ...
        meta:  (ac.owner||'') + (ac.priority ? ' · '+ac.priority : ''),
        nav:   'nav-actions'
      });
    });
  }
...
function calSelectDay(dateStr, dayEvts){
  ...
  if(!dayEvts.length){
    ...
    dayEvts.forEach(function(ev){
      ...
      item.title = 'Click to navigate to this section';
